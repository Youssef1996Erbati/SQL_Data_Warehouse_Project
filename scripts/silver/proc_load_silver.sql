...
===============================================================================
Objectif du script :
    Cette procédure stockée réalise le processus ETL (Extraction, Transformation, Chargement)
    afin d’alimenter les tables du schéma 'silver' à partir du schéma 'bronze'.

Actions effectuées :
    - Vide (TRUNCATE) les tables Silver.
    - Insère des données transformées et nettoyées depuis Bronze vers les tables Silver.
...
BEGIN
    ...
    BEGIN TRY
		...
		-- Chargement silver.crm_cust_info
		...
		SELECT
			...
			CASE 
				...
				ELSE 'n/a'
			END AS cst_marital_status, -- Normaliser les valeurs du statut marital dans un format lisible
			CASE 
				WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				ELSE 'n/a'
			END AS cst_gndr, -- Standardiser les valeurs du genre pour les rendre lisibles
			cst_create_date
		FROM (
			SELECT
				*,
		...
		WHERE flag_last = 1; -- Sélectionner le record le plus récent par client
		SET @end_time = GETDATE();
        PRINT '>> Durée de chargement: ' + CAST(DATEDIFF(SECOND, @start_time, @end_time) AS NVARCHAR) + ' secondes';
        PRINT '>> -------------';

		-- Chargement silver.crm_prd_info
        SET @start_time = GETDATE();
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Extraire category ID
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key,        -- Extraire product key
			prd_nm,
			...
			CASE 
				...
				ELSE 'n/a'
			END AS prd_line, -- Map les codes de ligne de produit à des valeurs descriptives
			CAST(prd_start_dt AS DATE) AS prd_start_dt,
			CAST(
				LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt) - 1 
				AS DATE
			) AS prd_end_dt -- Calculer la date de fin comme étant un jour avant la prochaine date de début
		FROM bronze.crm_prd_info;
        SET @end_time = GETDATE();
        PRINT '>> Durée de chargement: ' + CAST(DATEDIFF(SECOND, @start_time, @end_time) AS NVARCHAR) + ' secondes';
        PRINT '>> -------------';

        -- Chargement crm_sales_details
        SET @start_time = GETDATE();
		...
		SELECT 
			...
			CASE 
				...
				ELSE sls_price  -- Calculer le prix si la valeur d’origine est invalide
			END AS sls_price
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS gen -- Standardiser les valeurs du genre et gérer les cas inconnus
		...
		SELECT
			...
			CASE
				...
				ELSE TRIM(cntry)
			END AS cntry -- Standardiser et gérer les codes pays manquants ou vides
		...
		PRINT '=========================================='
		PRINT 'Chargement de la couche Silver est terminé';
